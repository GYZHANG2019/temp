...
static int usage(char **argv)
{
    ...
    printf("example:\n");
    printf("\t%s /dev/transcoder0 rc2ep 1 32 ---> rc2ep transfer 1 times data, each data size is 32MB with /dev/transcoder0\n", argv[0]);
    printf("\t%s /dev/transcoder1 ep2rc 2 64 ---> ep2rc transfer 2 times data, each data size is 64MB with /dev/transcoder1\n", argv[0]);
}
...
int main(int argc, char **argv)
{
	...
	float bw=0;
	unsigned long special_epaddr = 0x4200000;
    ...
    do {
    	...
    	if(strncmp(argv[2],"rc2ep", 5) == 0) {
            ...
            printf("[ERROR] please input correct transfer direction param, invalid param: %s\n", argv[2]);
            ret = -3;
            break;
        }
        count = strtoul(argv[3], 0, 0);
        if (count <= 0) {
            printf("[ERROR] please input correct count number param, invalid param: %s\n",argv[3]);
            ret = -4;
            break;
        }
		size = strtoul(argv[4], 0, 0);
        if (size <= 0) {
            printf("[ERROR] please input correct data size param, invalid param: %s\n",argv[4]);
            ret =-5;
            break;
        }
		if(size > 64)
        ...
        if (fd == -1) {
            printf("[ERROR] failed to open: %s, please check the device node is exist...\n",argv[1]);
            break;
        }
	...
	if (argc == 6) {
		special_epaddr = strtoul(argv[5], 0, 0);
	} else {
		...
		special_epaddr = ep_region.phy_addr;
	}
    	printf("EP address=0x%lx \n",special_epaddr);

    	if (DIRECTION_RC2EP == direction) {
    		src = rc_region.phy_addr;
    		dst = special_epaddr; // can fix a special addr.
    		...
    		src = special_epaddr; // can fix a special addr.
        	src_l = src & 0xffffffff;
        	src_h = src >>32;
        	dst_l = dst & 0xffffffff;
        	dst_h = dst >>32;
        ...
        for (i=0; i<count; i++) {
            ...
            ioctl(fd,CB_TRANX_EDMA_TRANX,&edma_trans);//enable edma transmit
            gettimeofday(&time_1,NULL);
            diff = 1000000 * (time_1.tv_sec-time.tv_sec) + time_1.tv_usec-time.tv_usec;
            time_cnt += diff;
