...
{
    printf("usage:\n");
    printf("\t%s device_node transfer_direction test_count size_Byte\n",argv[0]);
    printf("\tdevice_node       : /dev/transcoderN\n");
    printf("\ttransfer_direction: rc2ep | ep2rc\n");
    printf("\ttest_count        : transfer times\n");
    printf("\tsize              : transfer data size in Byte(1~)\n");
    printf("example:\n");
    printf("\t%s /dev/transcoder0 rc2ep 1 32 ---> rc2ep transfer 1 times data, each data size is 32Byte with /dev/transcoder0\n", argv[0]);
    printf("\t%s /dev/transcoder1 ep2rc 2 64 ---> ep2rc transfer 2 times data, each data size is 64Byte with /dev/transcoder1\n", argv[0]);
}
...
int main(int argc, char **argv)
{
    ...
    do {
        ...
        for (i=0; i<count; i++) {
            ...
            if (ioctl(fd,CB_TRANX_EDMA_TRANX,&edma_trans) < 0)//enable edma transmit
        ...
        bw = count*size*1000*1000/time_cnt/1024/1024;
