...
static void usage()
{
    printf("usage: ./pcie_ddr_memtest <d:device> [r | w | t] [i] [E] [n] [s] [S] [I] [filename]\n");
    ...
    printf("\t\t./pcie_ddr_memtest -d /dev/transcoder0 -I -s <size> -n <loop>\n");
	
//    printf("\t\t./pcie_ddr_memtest -d /dev/transcoder0 -I [-t] -s <size> -n [filename]\n");
//    printf("\t\t./pcie_ddr_memtest -d /dev/transcoder0 -I [-r|-w] <ep_addr> -s <size> filename\n");
}
...
void get_time(char *s)
{
    ...
    printf("[%s] %d-%d-%d %d:%d:%d\n", s, (p->tm_year + 1900), (1+p->tm_mon), p->tm_mday, p->tm_hour, p->tm_min, p->tm_sec);
}

static int compare_data(struct info_rc *rc, u32 size, u32 ep_addr, unsigned long ep_end_addr)
{
    u32 i;
    ...
    printf("[memtest]\033[32m compare pass start addr: 0x%08x, end addr: 0x%lx !!!\033[0m\n", ep_addr, ep_end_addr);
exit:
    return 0;
}
...
static int malloc_rc(struct info_rc *rc, u32 size, int fd)
{
	...
	rc->va = mmap(0, map_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, rc_region->phy_addr & ~MAP_MASK);
#else
...
static int do_trans_test(struct t_info *ti)
{
    ...
    do {
        printf("[memtest] trans %d begin, ddr_addr_start: 0x%x, size: 0x%x, ddr_end_addr: 0x%lx.\n", count++,  ep_addr, size,size+ep_end_addr);
...
//            edma_trans_wr.fix_channel = -1;
            //printf("[memtest] RC2EP 0x%lx(0x%x) -> 0x%08x, size: 0x%08x\n", ti->rc[0].pa, edma_trans_wr.sar_low, i, size);
...
//            edma_trans_rd.fix_channel = -1;
            //printf("[memtest] RC2EP 0x%08x -> 0x%lx(0x%x), size: 0x%08x\n", i, ti->rc[0].pa, edma_trans_rd.dar_low, size);
...
int main(int argc, char **argv) 
{
	...
	trans_info->fd_dev = open(trans_info->tp.dev_node, O_RDWR, S_IRUSR|S_IWUSR);
...
exit:
	...
	if(trans_info->rc[0].va) {
        ...
        ioctl(trans_info->fd_dev, CB_TRANX_MEM_FREE, &trans_info->rc[0].rc_region);		
    }
	if(trans_info->rc[1].va) {
		munmap(trans_info->rc[1].va, trans_info->tp.size);
        ioctl(trans_info->fd_dev, CB_TRANX_MEM_FREE, &trans_info->rc[1].rc_region);		
    }
#else
	if(trans_info->rc[0].va)
